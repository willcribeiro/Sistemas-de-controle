 ...
 * External functions *
 *====================*/
EXTERN RT_MODEL * coupled_tanks_filtro_wind(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(coupled_tanks_filtro_wind_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://coupled_tanks_filtro_wind:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: coupled_tanks_filtro_wind -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
