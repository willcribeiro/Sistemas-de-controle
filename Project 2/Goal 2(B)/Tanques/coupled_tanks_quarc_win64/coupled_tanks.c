 ...
 * C source code generated on : Fri Sep 20 16:28:46 2019
 *
...
void coupled_tanks_output(void)
{
  ...
  if (rtmIsMajorTimeStep(coupled_tanks_M)) {
    ...
    coupled_tanks_B.Compare = (coupled_tanks_B.VoltstocmTank1 <=
      coupled_tanks_P.LowlevelLL_const);

    /* Constant: '<Root>/Constant' */
    coupled_tanks_B.Referencia = coupled_tanks_P.Constant_Value;

    /* Sum: '<Root>/Sum' */
    coupled_tanks_B.Sum = coupled_tanks_B.Referencia -
   ...
   */
  coupled_tanks_B.Ki = coupled_tanks_P.Ki_Gain *
  ...
  if ((coupled_tanks_DW.TimeStampA >= coupled_tanks_M->Timing.t[0]) &&
    ...
    rtb_Derivative = coupled_tanks_DW.TimeStampA;
    ...
    if (coupled_tanks_DW.TimeStampA < coupled_tanks_DW.TimeStampB) {
      if (coupled_tanks_DW.TimeStampB < coupled_tanks_M->Timing.t[0]) {
        rtb_Derivative = coupled_tanks_DW.TimeStampB;
    ...
    rtb_Derivative = (coupled_tanks_B.Sum - *lastU) / (coupled_tanks_M->
      Timing.t[0] - rtb_Derivative);
  }

  /* End of Derivative: '<Root>/Derivative' */

  /* Gain: '<Root>/Kd ' */
  coupled_tanks_B.Kd = coupled_tanks_P.Kd_Gain * rtb_Derivative;

  /* Sum: '<Root>/Add' */
  coupled_tanks_B.Add = (coupled_tanks_B.Kp + coupled_tanks_B.Ki) +
    coupled_tanks_B.Kd;
  if (rtmIsMajorTimeStep(coupled_tanks_M)) {
    /* RelationalOperator: '<S4>/Compare' incorporates:
     *  Constant: '<S4>/Constant'
     */
    coupled_tanks_B.Compare_b = (coupled_tanks_B.VoltstocmTank1 >=
      coupled_tanks_P.CompareToConstant2_const);
  }

  /* MultiPortSwitch: '<S2>/Multiport Switch' incorporates:
   *  Constant: '<S2>/Desliga a bomba(0V)'
   *  Constant: '<S2>/OP Normal'
   *  Constant: '<S2>/Tensao que mantém nivel 1 próximo de 30 cm'
  ...
  switch ((int32_T)((coupled_tanks_P.OPNormal_Value - (real_T)
                     (coupled_tanks_B.Compare ? coupled_tanks_B.Add <
                      coupled_tanks_P.CompareToConstant1_const : 0)) - (real_T)
                    (coupled_tanks_B.Add >= coupled_tanks_P.HighLevel_const ?
                     (int32_T)coupled_tanks_B.Compare_b : 0) * (real_T)
                    coupled_tanks_P.Gain_Gain * 6.103515625E-5)) {
   case 1:
    coupled_tanks_B.MultiportSwitch =
      coupled_tanks_P.Tensaoquemantmnivel1prximode30c;
    break;

   case 2:
    coupled_tanks_B.MultiportSwitch = coupled_tanks_P.Desligaabomba0V_Value;
    break;

   default:
    /* Saturate: '<S2>/Saturation' */
    if (coupled_tanks_B.Add > coupled_tanks_P.Saturation_UpperSat) {
      coupled_tanks_B.MultiportSwitch = coupled_tanks_P.Saturation_UpperSat;
    } else if (coupled_tanks_B.Add < coupled_tanks_P.Saturation_LowerSat) {
      coupled_tanks_B.MultiportSwitch = coupled_tanks_P.Saturation_LowerSat;
    } else {
      coupled_tanks_B.MultiportSwitch = coupled_tanks_B.Add;
    }
  ...
  if (rtmIsMajorTimeStep(coupled_tanks_M)) {
    ...
    if (coupled_tanks_B.VoltstocmTank2 >=
        coupled_tanks_P.CompareToConstant4_const) {
      rtmSetStopRequested(coupled_tanks_M, 1);
    }
...
RT_MODEL_coupled_tanks_T *coupled_tanks(void)
{
  ...
  rtmSetTFinal(coupled_tanks_M, 200.0);
  coupled_tanks_M->Timing.stepSize0 = 0.002;
  coupled_tanks_M->Timing.stepSize1 = 0.002;

  /* External mode info */
  coupled_tanks_M->Sizes.checksums[0] = (3577733649U);
  coupled_tanks_M->Sizes.checksums[1] = (3851665607U);
  coupled_tanks_M->Sizes.checksums[2] = (3574575342U);
  coupled_tanks_M->Sizes.checksums[3] = (4054157592U);
  ...
  coupled_tanks_M->Sizes.numBlocks = (40);/* Number of blocks */
  coupled_tanks_M->Sizes.numBlockIO = (11);/* Number of block outputs */
