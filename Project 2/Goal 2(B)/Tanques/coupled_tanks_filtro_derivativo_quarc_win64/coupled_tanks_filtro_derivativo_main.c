...
typedef RT_MODEL_coupled_tanks_filtro_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * coupled_tanks_filtro_derivativo(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(coupled_tanks_filtro_derivat_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://coupled_tanks_filtro_derivativo:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: coupled_tanks_filtro_derivativo -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
   ...
   ************************/
  S = coupled_tanks_filtro_derivativo();
  if (rtmGetErrorStatus(S) != NULL) {
